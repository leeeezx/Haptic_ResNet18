...
def add_noise(data, noise_type='gaussian', noise_level=0.1):
    ...
    Args:
        data (np.ndarray): 输入数据, shape: (sequence_length, 2)
...
def preprocess_data(all_data, all_labels, length_config, noise_type='gaussian', noise_level=0.1):
    """
    对加载的数据进行预处理,包括添加噪声、截取、填充和格式转换
    ...
    print(f"添加 {noise_type} 噪声 (level={noise_level})...")
    
    # 对所有样本添加噪声并进行截取和填充
    processed_data = []
    for idx, sample in enumerate(all_data):
        # # 首先为每个CSV样本添加噪声
        # noisy_sample = add_noise(sample, noise_type=noise_type, noise_level=noise_level)
...
def main():
    ...
    all_data, all_labels, _, _, _ = load_data(DATA_ROOT)
    X_test, y_test = preprocess_data(all_data, all_labels, length_config)
    ...
    print("噪声测试总结")
    print("=" * 50)
    print(f"{'噪声类型':<15} {'噪声水平':<10} {'准确率':<10}")
    print("-" * 35)
